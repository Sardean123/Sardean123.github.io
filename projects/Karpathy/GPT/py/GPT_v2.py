...
with open('Seus.txt', 'r', encoding='utf-8') as f:
  text = f.read()
...
def get_batch(split):
    ...
    ix = torch.randint(len(data) - block_size, (batch_size,)) # starting indices of size batch_size
...
class MultiHeadAttention(nn.Module):
  """ Multiple heads of self-attention in parallel, batched"""

  def __init__(self, n_head, n_embed):
      super().__init__()
      # key, query, value projections for all heads, but in a batch
      self.cross_attention = nn.Linear(n_embed, 3 * n_embed, bias=False)
      # output projection
  ...
  def forward(self, x):
    ...
    q, k, v = self.cross_attention(x).split(self.n_embed, dim=2)
...
torch.save(model, 'GPT_seus_model.pth')
